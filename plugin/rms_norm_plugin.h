...
namespace
{
static const char *PLUGIN_NAME {"RMSNorm"};
...
class RMSNormPlugin : public IPluginV2DynamicExt
{
 ...
 public:
  ...
  RMSNormPlugin(const std::string &name, const void *buffer, size_t length):
    name_(name)
  {
  ...
  IPluginV2DynamicExt *clone() const noexcept override
  {
    WHERE_AM_I();
		...
		LOG_INFO("length is " + std::to_string(length_));
		LOG_INFO("gamma is " + std::to_string(h_gamma[0]));
    return new RMSNormPlugin(name_, epsilon_, length_, h_gamma);
  }
 ...
 public:
  ...
  IPluginV2 *createPlugin(const char *name, const PluginFieldCollection *fc) noexcept override
  {
    ...
    for (int i = 0; i < fc->nbFields; i++)
    {
		...
		LOG_INFO("embedding length is " + std::to_string(length));
		LOG_INFO("gamma is " + std::to_string(gamma[0]));
    LOG_INFO("====================================");
    return new RMSNormPlugin(name, epsilon, length, gamma);
  }

  IPluginV2 *deserializePlugin(const char *name, const void *serialData, size_t serialLength) noexcept override
  {
    WHERE_AM_I();
    return new RMSNormPlugin(name, serialData, serialLength);
  }
