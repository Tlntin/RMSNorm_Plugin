...
# input_path = os.path.join(data_dir, "hidden_states.npy")
...
def export_model():
    ...
    input_data = torch.rand(
        max_seq_length, batch_size, embedding_size, dtype=torch.float32).to(device)
    ...
    torch.onnx.export(
        ...
        opset_version=14,
        dynamic_axes={
            "input": {1: "seq_length", 0: "batch_size", },
        }
    )
    print("模型导出成功")
...
def build_engine(onnx_path1: str):
    ...
    profile.set_shape(
        ...
        opt=[max_seq_length, batch_size,  embedding_size],
        max=[max_seq_length * 2, batch_size * 2, embedding_size]
    )
...
def inference():
    ...
    raw_h_input = np.load(input_path)
    h_input = np.ascontiguousarray(raw_h_input.reshape(-1)).astype(np.float32)
    print("raw input shape", raw_h_input.shape) 
    context.set_input_shape(input_name, raw_h_input.shape)
